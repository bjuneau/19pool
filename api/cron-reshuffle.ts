...
export default async function handler(req: CronRequest, res: CronResponse) {
    ...
    if (!secret || secret.length === 0) {
        // Distinguishes an absent variable from one saved with a blank value,
        // which is the failure mode that actually happened and is invisible
        // from the dashboard.
        console.warn(
            `[cron-reshuffle] CRON_SECRET unusable: present=${
                'CRON_SECRET' in process.env
